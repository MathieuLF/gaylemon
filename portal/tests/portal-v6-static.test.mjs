...
test("les parcours publics exposent les nouveaux contrôles accessibles", async () => {
  ...
  assert.match(app, /function renderDailyBriefItem/);
  assert.match(app, /function renderDailyBasicPlayerCard/);
  assert.match(app, /aria-label="\$\{escapeHtml\(`\$\{label\}: \$\{detail\}`\)\}"/);
  assert.match(app, /renderDailyBriefItem\("Journal", "Rien de notable publié pour cette journée\."\)/);
  assert.match(app, /players\.map\(\(player\) => renderDailyBasicPlayerCard\(player\)\)/);
  assert.match(styles, /daily-brief__list li[\s\S]*?grid-template-columns:\s*minmax\(126px,/);
  assert.match(app, /daily-type-strip[\s\S]*?<b>\$\{escapeHtml\(entry\.name\)\}<\/b><small>\$\{escapeHtml\(dailyAggregateQuantityLabel\(entry\)\)\}<\/small>/);
  assert.match(styles, /daily-type-strip span[\s\S]*?display:\s*inline-flex;[\s\S]*?gap:\s*7px/);
  assert.match(app, /<small>Niveaux gagnés<\/small>/);
  assert.match(app, /<small>Pals repérés<\/small>/);
  assert.match(app, /<small>Objets fabriqués<\/small>/);
  assert.match(app, /<small>Ressources produites<\/small>/);
  assert.match(app, /<small>Moments<\/small>/);
  assert.match(app, /<small>Faits divers<\/small>/);
  assert.doesNotMatch(app, /hasPublishedEchoes/);
  assert.doesNotMatch(app, /<small>Sessions du jour<\/small>/);
  assert.doesNotMatch(app, /<small>Niveau actuel<\/small>/);
  ...
  assert.match(app, /function publicEventGuildName/);
  assert.match(app, /function eventLineIdentityBadges/);
  assert.match(app, /event-line__guild/);
  assert.match(app, /event\?\.type === "research"/);
  assert.match(app, /const inlineMeta = `<span class="event-line__meta">/);
  ...
  assert.match(app, /headline: detailHeadline/);
  assert.match(app, /const visibleBullets = terminalRoute \? \[\] : bullets/);
  assert.match(app, /const detailClass = visibleBullets\.length \? " event-line--with-bullets" : ""/);
  assert.match(app, /visibleBullets\.length \? `<ul class="event-line__bullets">/);
  assert.match(app, /visibleBullets\.map/);
  assert.match(app, /<button class="event-line__more" type="button"/);
  assert.match(app, /aria-controls="\$\{escapeHtml\(tooltipId\)\}" aria-expanded="false"/);
  assert.match(app, /function closeOpenEventDetails/);
  assert.match(app, /function toggleEventDetail/);
  assert.match(app, /document\.activeElement\.blur\(\)/);
  assert.match(app, /eventStream\?\.addEventListener\("click"[\s\S]*?toggleEventDetail\(line\)/);
  assert.match(app, /document\.addEventListener\("click"[\s\S]*?closeOpenEventDetails\(\)/);
  assert.match(app, /document\.addEventListener\("keydown"[\s\S]*?event\.key !== "Escape"[\s\S]*?closeOpenEventDetails\(\)/);
  assert.match(app, /class="event-line__tooltip"/);
  assert.match(app, /event-line__tooltip-items/);
  assert.match(styles, /--terminal-visible-echoes:\s*6/);
  ...
  assert.match(styles, /event-line__content[\s\S]*?padding-inline-end:\s*0/);
  assert.match(styles, /event-line__meta[\s\S]*?display:\s*flex/);
  assert.match(styles, /event-line__meta \.event-line__player,[\s\S]*?event-line__meta \.event-line__guild/);
  assert.match(styles, /event-line__more[\s\S]*?display:\s*none/);
  assert.match(styles, /event-line__more[\s\S]*?display:\s*inline-flex/);
  assert.match(styles, /@media \(hover: hover\) and \(pointer: fine\)/);
  assert.match(styles, /event-line:focus-visible \.event-line__more/);
  assert.match(styles, /event-line\.is-detail-open \.event-line__more/);
  assert.match(styles, /event-line:focus-visible \.event-line__tooltip/);
  assert.match(styles, /event-line\.is-detail-open \.event-line__tooltip/);
  assert.match(styles, /event-line__more[\s\S]*?min-inline-size:\s*48px/);
