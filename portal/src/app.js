import { captureFocus, restoreFocus, replaceContentPreservingFocus } from "./shared/focus.js";
import { clamp, escapeHtml, escapeRegExp, playerSlug, playerInitials, parseDate, formatInteger, formatCompactDuration, formatPercent, formatProgressPercent, formatBytes, normalizeEventSearch } from "./shared/format.js";
import { hueDistance, playerColor, playerColorInk } from "./shared/player-colors.js";
import { catalogExperienceProgress, catalogFriendshipProgress } from "./players/progression.js";
import { publicEventOrderRank, eventAggregationWindowMinutes, eventAggregationWindowLabel, eventAggregationHeadline } from "./terminal/event-labels.js";
import { createJsonReader } from "./shared/data.js";
import { createMapController } from "./map/controller.js";
import { createLeaderboards } from "./leaderboards/view.js";
import { createDailyTypesRenderer } from "./daily/type-filters.js";
...
const mapState = { expandedFallback: false };
...
const readJson = createJsonReader(() => seasonBasePath);
...
const { applyGlobalMapView, restoreGlobalMapView, toggleGlobalMapFullscreen } = createMapController({
  globalMapViewport, globalMapScene, globalMapZoom, globalMapImage, globalMapFigure,
  loadDeferredImage, syncGlobalMapControls, mapState,
});
...
function isKnownPlayerName(value) {
  ...
  if (/^(joueur|player)(?:[\s#_-]*(?:\d+|inconnu|unknown|non identifie|non attribue))?$/.test(key)) return false;
  return ![
    "inconnu",
    "unknown",
...
function playerDisplayName(value) {
  ...
  return isKnownPlayerName(name) ? name : "";
}

function isPlaceholderPlayerName(value) {
  return !isKnownPlayerName(value);
}
...
function renderHeaderPlayersTooltip() {
  ...
  setMetric("players-current", count);
  const accessibleLabel = count
    ? players.map((player) => {
      ...
      return `${playerDisplayName(player.name) || "Aventurier"}, arrivée ${arrival}, ${formatPresenceDurationSince(startedAt, now)}`;
    }).join("; ")
  ...
  playersTooltip.innerHTML = `
    ...
    return `
        <li style="--player-color:${escapeHtml(playerColor(player))}">
          ...
          <span><b>${escapeHtml(playerDisplayName(player.name) || "Aventurier")}</b><small>Arrivée ${escapeHtml(arrival)} · ${escapeHtml(durationText)}</small></span>
        </li>`;
    }).join("")}
    </ul>
    `;
...
function createProvisionalPlayer(activity) {
  const name = playerDisplayName(activity?.name);
  if (!name) return null;
  return {
    name,
...
function getDisplayPlayers(payload) {
  const savedPlayers = (Array.isArray(payload?.players) ? payload.players : [])
    ...
    .filter((player) => isKnownPlayerName(player?.name));
  ...
  getPlayersCollection(statsSnapshot).forEach((activity) => {
    ...
    if (!activity?.isOnline || !name || !slug || playersBySlug.has(slug)) return;
    const provisionalPlayer = createProvisionalPlayer({ ...activity, name });
    if (provisionalPlayer) playersBySlug.set(slug, provisionalPlayer);
  });
...
function getPlayerActivity(player) {
  return playerActivityByName.get(normalizePlayerNameKey(player?.name)) || null;
}
...
const { renderLeaderboards, rankingColumn, leaderboardCategories } = createLeaderboards({
  leaderboardCategory, leaderboardSearch, leaderboardPresence, leaderboardBody, leaderboardHead, leaderboardPodium, leaderboardResultCount, getPlayerActivity, playerRoute, formatLastSeen, formatPing,
  getSnapshot: () => saveSnapshot, getSort: () => ({ key: leaderboardSortKey, direction: leaderboardSortDirection }),
});
...
function renderUptime(payload) {
  ...
  const gameUptimeSeconds = monitor?.uptimeSeconds ?? summary.uptimeSeconds ?? summary.gameUptimeSeconds;
  ...
  uptimeSummary.textContent = payload.source === "palworld-rest-api"
    ...
    : `${getStatusSentence(status)} · ${formatPercent(uptime)} de disponibilité sur 24 h${pingText}`;
  ...
  uptimeBars?.setAttribute("aria-label", payload.source === "palworld-rest-api"
    ? "Dernier contrôle de l'API REST Palworld"
...
function renderStats(payload) {
  ...
  const players = getPlayersCollection(payload).filter((player) => isKnownPlayerName(player?.name));
  const server = payload.server || {};
  statsSnapshot = { ...payload, players };
  playerActivityByName = new Map(players.map((player) => [normalizePlayerNameKey(player.name), player]));
...
// Player colors are generated HSL values; choose the more legible neutral ink.
...
function syncGlobalMapControls() {
  ...
  const expanded = mapState.expandedFallback || Boolean(document.fullscreenElement && document.fullscreenElement === globalMapFigure);
...
function renderGlobalPlayerMap(players, bases = currentBasesSnapshot()?.bases || []) {
  ...
  const playerMarkers = showMapPlayers ? positioned.map(({ player, index }, markerIndex) => {
    ...
    return `
      <span class="global-player-marker${onlineClass}" role="img"
        ...
        style="left:${Number(position.leftPercent)}%;top:${Number(position.topPercent)}%;--marker-color:${color};--marker-ink:${playerColorInk(color)};--marker-order:${markerIndex}"
  ...
  const playerLegend = showMapPlayers ? positioned.map(({ player, index }) => {
    ...
    const color = playerColor(player);
    return `
      <a class="global-player-legend-row" href="${playerRoute(player)}" data-player-index="${index}" data-player-slug="${playerSlug(player.name)}" style="--marker-color:${color};--marker-ink:${playerColorInk(color)}">
  ...
  const unchartedLegend = showMapPlayers ? uncharted.map(({ player, index }) => {
    ...
    const color = playerColor(player);
    return `
      <a class="global-player-legend-row global-player-legend-row--uncharted" href="${playerRoute(player)}" data-player-index="${index}" data-player-slug="${playerSlug(player.name)}" style="--marker-color:${color};--marker-ink:${playerColorInk(color)}">
        <span class="global-player-legend-row__marker">${escapeHtml(playerInitials(player.name))}</span>
...
function renderSaveSnapshot(payload, syncRoute = true) {
  ...
  if (savePlayers) replaceContentPreservingFocus(savePlayers, players.map((player, index) => {
    const pals = player.pals || {};
...
function dailyTotalStructures(metrics) {
  return Number(metrics.build || 0) + Number(metrics.repair || 0);
}

function dailyStructureDetail(metrics) {
  const structures = Number(metrics.build || 0);
  const repairs = Number(metrics.repair || 0);
  if (structures && repairs) return `${dailyPlural(structures, "structure ajoutée", "structures ajoutées")} · ${dailyPlural(repairs, "réparation", "réparations")}`;
  if (structures) return dailyPlural(structures, "structure ajoutée", "structures ajoutées");
  if (repairs) return dailyPlural(repairs, "réparation", "réparations");
  return "aucune base modifiée";
}

function dailyOtherFactsDetail(metrics) {
  const details = [
    Number(metrics.boss || 0) ? dailyPlural(metrics.boss, "boss", "boss") : "",
    Number(metrics.discovery || 0) ? dailyPlural(metrics.discovery, "découverte", "découvertes") : "",
    Number(metrics.challenge || 0) ? dailyPlural(metrics.challenge, "défi", "défis") : "",
    Number(metrics.mutation || 0) ? dailyPlural(metrics.mutation, "mutation", "mutations") : "",
  ].filter(Boolean);
  return details.length ? details.join(" · ") : "aucun autre moment";
}
...
const renderDailyTypes = createDailyTypesRenderer({ dailyTopAggregates, dailyConsolidatedPalFinds, dailyPalSignalTotal, dailyAggregatePlayersLabel, dailyAggregateQuantityLabel, dailyAggregateShortKind, gameImage, getActiveFilter: () => dailyHighlightTypeFilter });

function syncDailyTypeFilters() {
  dailyTypes?.querySelectorAll("[data-daily-type-filter]").forEach((button) => {
    const active = button.dataset.dailyTypeFilter === dailyHighlightTypeFilter;
    button.setAttribute("aria-pressed", String(active));
    button.closest(".daily-tangible-card")?.classList.toggle("is-active", active);
  });
}
...
function renderDailyHighlights(summary) {
  const highlights = dailyCuratedEventHighlights(summary, 18)
  ...
  return highlights.map((highlight) => {
    ...
    const badgeTitle = highlight.confidence === "derived" ? ' title="Ce moment vient de la guilde; le joueur affiché est le meilleur repère disponible."' : "";
    return `
...
function setDailyActivityVisibility(hasActivity) {
  document.querySelectorAll(".daily-overview, .daily-section").forEach((section) => { section.hidden = !hasActivity; });
  dailyMetrics?.classList.toggle("is-empty", !hasActivity);
  if (!hasActivity && dailyMetrics) {
    dailyMetrics.innerHTML = '<p class="daily-empty">Aucune activité publiée pour cette journée. Choisis une autre date pour retrouver les moments de l’aventure.</p>';
  }
  return hasActivity;
}
...
function renderDailyDigest(summary) {
  ...
  const hasActivity = Boolean(totals.eventCount || (summary.presenceAvailable !== false && totals.presenceSessions));
  if (!setDailyActivityVisibility(hasActivity)) return;
  const palSignals = dailyPalSignalTotal(summary);
  ...
  if (dailyTypes) {
    replaceContentPreservingFocus(dailyTypes, renderDailyTypes(summary));
  }
  if (dailyPlayers) {
...
function renderDailyV6Basic(payload) {
  ...
  dailyCurrentSummary = null;
  dailyHighlightTypeFilter = "";
  if (!setDailyActivityVisibility(echoes > 0)) {
    if (dailyStatus) dailyStatus.textContent = "Aucun moment publié pour cette journée";
    return;
  }
  ...
  if (dailyHighlights) dailyHighlights.innerHTML = latest.length
    ? latest.map((event) => {
      ...
      const badgeTitle = event.confidence === "derived" ? ' title="Ce moment vient de la guilde; le joueur affiché est le meilleur repère disponible."' : "";
    ...
    return `
      <article class="pal-detail-card">
        ...
        <div class="pal-detail-card__body">
          ...
          ${palExperience ? `<div class="pal-progression"><span><small>Prochain niveau</small><strong>${Number(palExperience.remaining || 0).toLocaleString("fr-CA")} EXP</strong></span><i role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${clamp(Number(palExperience.percent || 0), 0, 100)}" aria-label="${escapeHtml(pal.name)} : progression vers le niveau ${Number(palExperience.nextLevel || pal.level + 1)}"><em style="width:${Math.max(0, Math.min(100, Number(palExperience.percent || 0)))}%"></em></i></div>` : ""}
          ...
          ${(computed.attack != null || computed.defense != null || computed.workSpeed != null) ? `
            <div class="pal-computed-stats" role="group" aria-label="Statistiques calculées">
...
async function openPlayerDetails(index, tab = "profile", updateRoute = true) {
  ...
  if (willOpenDialog) {
    playerDialogReturnFocus = opener;
    lockPlayerView();
  }
...
function closePlayerDetails() {
  ...
  window.requestAnimationFrame(() => {
    restoreFocus(returnFocus, document, document.querySelector("#save-title"));
  });
}
...
async function loadEventsFreshness(silent = true, force = false) {
	try {
		const payload = await readJson("api/public/events/v1?limit=1&offset=0");
		if (!payload?.ok || payload.source !== "postgresql" || !Array.isArray(payload.events)) {
			throw new Error("invalid-database-events");
		}
...
document.querySelectorAll("[data-detail-tab]").forEach((button) => {
  ...
  button.addEventListener("keydown", (event) => {
    ...
    event.preventDefault();
    const tabs = [...document.querySelectorAll("[data-detail-tab]:not(:disabled)")];
...
dailyHighlights?.addEventListener("click", (event) => {
  ...
  dailyHighlightTypeFilter = "";
  syncDailyTypeFilters();
  dailyTypes?.querySelector("[data-daily-type-filter]")?.focus({ preventScroll: true });
